...
import javax.swing.*;
import java.sql.*;
public class UpdateAccountant extends Frame implements ActionListener{
	...
	public UpdateAccountant(){
		super("Update Accountant Window");
	...
	public void loadData(){
		String sql="select AC_Name from accountant where AC_Id='"+id.getText()+"'";
		...
		try{
			...
			while(rs.next()){
				...
				data=data+rs.getString("AC_Name")+"\n";
			}
			name.setText(data);
		}
		catch(Exception ex){
	...
	public void actionPerformed(ActionEvent e){
		...
		if(sig.equals("Confirm")){
			String m="";
			String sql="update accountant set AC_Name='"+name.getText()+"',PhoneNo='"+phn.getText()+"',Password='"+password.getText()+"' where AC_Id='"+id.getText()+"'";
			System.out.println(sql);
			DataAccessDoctor da2=new DataAccessDoctor();
			if(isEmpty(name)){
